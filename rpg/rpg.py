...
class Game(Data):
    ...
    def move_to(self, newloc=None):  #2
        ...
        dwarf_blocking_the_way = any(
            dwarf.old_room is newloc and dwarf.has_seen_rpgr
    ...
    def move_dwarves(self):
        ...
        for dwarf in self.dwarves + [ self.pirate ]:
            ...
            if self.loc in (dwarf.room, dwarf.old_room):
                dwarf.has_seen_rpgr = True
            elif self.loc.is_before_hall_of_mists:
                dwarf.has_seen_rpgr = False

            if not dwarf.has_seen_rpgr:
                continue
    ...
    def dispatch_command(self, words, ctx, rpg):  #19999
        ...
        if words[0] == 'save' and len(words) <= 2:
            ...
            team = rpg.players[server.id][channel.id][author.id]
            if len(words) == 2:
                save = words[1]
            else:
                save = rpg.game_loops[server.id][team][channel.id]["SAVE"]
            if save is None:
                return self.write("There is no save file to save to. Please give a name for the save file\n `{}> save <save_name>`".format(ctx.prefix))
            return rpg.save_game(server, team, channel, save)
            # return self.t_suspend(words[0], 'data/rpg/saves/{}/{}.save'.format(ctx.message.author.id, words[1]))
        ...
        if word1 == 'say':
            return self.t_say(word1, word2, ctx, rpg) if word2 else self.i_say(word1)
            ...
            if not obj_here:
                if obj is self.grate:
                    if self.loc.n in (1, 4, 7):
                        return self.dispatch_command([ 'depression' ], ctx, rpg)
                    elif 9 < self.loc.n < 15:
                        return self.dispatch_command([ 'entrance' ], ctx, rpg)
                elif noun == 'dwarf':
    ...
    def t_say(self, verb, word, ctx, rpg):  #9030
        if word.n in (62, 65, 71, 2025):
            self.dispatch_command([ word.text ], ctx, rpg)
        else:
...
def install_words(game):
    # stack()[0] is this; stack()[1] is rpg.play(); so, stack()[2]
...
class RPG:
    ...
    async def embark(self, ctx, team: str=None, save: str=None):
        ...
        if server.id not in self.teams:
            ...
            msg += ", the first rpgr in " + server.name + ","
            first = True
        ...
        try:
            if self.game_loops[server.id][team][channel.id].get("PLAYING",False):
                ...
                await self.bot.reply('The {} team already has embarked on an rpg in this channel. Any unsaved progress will be lost. Type `overwrite` to discard your unsaved progress.'.format(tname))
    ...
    async def join(self, ctx, team=None):
            ...
            try:
                ...
                if cteams:
                    if len(cteams) == 1:
                        ...
                        await self.bot.reply('More than one of your teams have embarked on an rpg in this channel. Please specify which to join.')
                        return
                else:
                    await self.bot.reply('None of your teams have yet to embark on an rpg in this channel.')
                    return
            except:
                await self.bot.reply('There are no teams on rpgs in this channel.')
                return
        team = self._safe_path(team).lower()
        ...
        if author.id in self.players[server.id][channel.id] and team == self.players[server.id][channel.id][author.id]:
            await self.bot.say('{} must have spaced out and forgot that they were already on the rpg with the {} team..'.format(author.mention,tname))
            return
    ...
    async def _quit(self, ctx):
        ...
        try:
            ...
            await self.bot.say('{} left the {} team\'s rpg in this channel.'.format(author.mention, tname))
        except:
            await self.bot.say('{} You are not part of any rpg in this channel'.format(author.mention))

    @rpg.command(pass_context=True, name='info')
    async def rpg_info(self, ctx):
        """Information about your current rpg in this channel"""
        author = ctx.message.author
        server = ctx.message.server
        channel = ctx.message.channel
        try:
            ...
            msg = "You are not a part of any rpg in this channel. "
            if server.id in self.players and channel.id in self.players[server.id]:
                if server.id in self.teams and author.id in self.teams[server.id]["MEMBERS"]:
                    msg += "If your team has embarked on an rpg in this channel, type `{}rpg join [team_name]` to join your teammates!".format(ctx.prefix)
                else:
        ...
        msg = "The {} team has embarked on an rpg in the {} channel".format(tname, channel.mention)
        if save is not None:
    ...
    async def team_saves(self, ctx, team=None):
        ...
        try:
            ...
            if len(files) > 1:
                msg += 's'
            reg = re.compile('data/rpg/saves/{}/{}/([^/]*).save'.format(server.id,team)) # just bein verbose
    ...
    async def recruit(self, ctx, user: discord.Member, team=None):
            ...
            if len(self.teams[server.id]["MEMBERS"][author.id]) == 1:
                ...
                if len(teams) > 1:
                    try:
                        ...
                        await self.bot.say('{} You\'re not in an rpg in this channel and you\'re in multiple teams. Specify which team to recruit to.'.format(author.mention))
                        return
                else:
                    team = teams[0]
        ...
        try:
            ...
            self.delete_team(server, team)
            await self.bot.say('The {} team\'s campsite is overgrown and in ruins. It is not certain when they passed away. It is certain though that that team will rpg no more.'.format(tname))
            return
    ...
    async def leave(self, ctx, team):
        ...
        await self.bot.say('{} leaves the {} team to seek other rpgs'.format(author.mention,tname))

    @team.command(pass_context=True, name='new')
    async def team_new(self, ctx, team):
        await self.bot.reply('Under construction. use `!rpg embark <team_name>` instead')
        return
    ...
    async def rpg_command(self, ctx, *, text):
        ...
        if words:
            ...
            try:
                ...
                await self.bot.reply('You are not in an rpg. If your team has embarked on one, join them using `{}rpg join`, otherwise embark on your own rpg.'.format(ctx.prefix))
                return
...
def check_folders():
    folders = ("data/rpg", "data/rpg/saves")
...
def check_files():
    ...
    for file, default in files.items():
        if not os.path.isfile(base_path + file):
            print("Creating default rpg {}...".format(file))
